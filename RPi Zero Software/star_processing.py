...
def get_ncp_position(polaris_coordinates, yildun_coordinates, ov_cephei_coordinates, image):
  ...
  ncp_error = ncp_coordinates - image_center
...
def draw_vector_line(image, origin, vec, length=200, color=(0,255,0), thickness=1):
...
image = cv2.imread("/home/pck/PCK/polaris5.jpg")
#polaris_img = cv2.imread("/content/polaris5.jpg")

#generate_vectors(polaris_img, 100)

polaris_vectors = np.load('polaris_vectors.npy')
yildun_vectors = np.load('yildun_vectors.npy')
ov_cephei_vectors = np.load('ov_cephei_vectors.npy')
...
if image is None:
  ...
  star_centers = brightest_stars(25,image) #Number of stars to be found, image, threshold
  print(star_centers.shape)
  ...
  for index, star in enumerate(star_centers):
    ...
    found_polaris, img_angle = find_polaris(star, np.append(star_centers[:index],star_centers[(index+1):], axis=0), 25, polaris_vectors)
    if found_polaris == True:
      print("Polaris is in the image at: X =", star[0], "Y =", star[1])
      polaris_coordinates = star
      break
  print("IMG is at angle:", img_angle)

  if not polaris_coordinates.size == 0:
    for index, star in enumerate(star_centers):
      #print(index)
      #print(star_centers[:index])
      if check_star(star, np.append(star_centers[:index],star_centers[(index+1):], axis=0), 25, yildun_vectors, img_angle):
    ...
    for index, star in enumerate(star_centers):
      #print(index)
      #print(star_centers[:index])
      if check_star(star, np.append(star_centers[:index],star_centers[(index+1):], axis=0), 25, ov_cephei_vectors, img_angle):
  ...
  #print(polaris_coordinates)
  #print(yildun_coordinates)
  #print(ov_cephei_coordinates)

  #Distance between Polaris and Yildun in pixels
  if not polaris_coordinates.size == 0 and not yildun_coordinates.size == 0 and not ov_cephei_coordinates.size ==0:
    ...
    ncp_coordinates, ncp_error = get_ncp_position(polaris_coordinates, yildun_coordinates, ov_cephei_coordinates, image)
    print("\nNCP coordinates:", ncp_coordinates)
    print("NCP error from img center:", ncp_error)
